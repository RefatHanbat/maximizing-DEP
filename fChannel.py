...
def myf_locations(sys_param,param_locations):
    ...
    d_OW_range = sys_param["d_OR_range"]
...
def myf_channel(sys_param,locations,param_channel):
    ...
    res_SI = sys_param["sigma_square_SI"]

    PL_exp = sys_param["beta"]
    ...
    # distances = 10**(-3) * np.linalg.norm(positions_S - positions_D, axis=1)**(-PL_exp)

    #print(10**(-3) * np.sqrt(np.sum(np.power(positions_D[:,0] - positions_S[:,0],2),0))**(-PL_exp))
